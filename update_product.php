...
if(isset($_POST['update'])){
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         $message[] = '¡El tamaño de las imágenes es demasiado grande!';
      }else{
...
<section class="update-product">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <input type="number" min="0" max="9999999999" required placeholder="Ingrese el precio del producto" name="price" onkeypress="if(this.value.length == 10) return false;" class="box" value="<?= $fetch_products['price']; ?>">
      <span>Actualizar categoria</span>
      <select name="category" class="box" required>
         <option selected value="<?= $fetch_products['category']; ?>"><?= $fetch_products['category']; ?></option>
         <option value="main dish">Plato principal</option>
         <option value="fast food">Comida rapida</option>
   ...
   <?php
         ...
         echo '<p class="empty">No hay productor añadidos todavia!</p>';
      }
   ?>
